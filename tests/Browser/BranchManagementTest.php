...
class BranchManagementTest extends AdminBrowserTestCase
{
    ...
    /**
     * Negatif: Form cabang harus menolak submit jika field required kosong
     */
    public function testTambahCabangGagalKarenaFieldRequiredKosong(): void
    {
        $this->browse(function (Browser $browser) {
            $this->loginAsAdmin($browser);
            ...
            $browser->assertPresent('#branchForm')
                ->assertSee('nama')
                ->pause(500);
        });
    }
    ...
    /**
     * Negatif: Form cabang harus menolak submit jika nomor telepon tidak valid
     */
    public function testTambahCabangGagalKarenaNomorTeleponTidakValid(): void
    {
        $this->browse(function (Browser $browser) {
            $this->loginAsAdmin($browser);
            ...
            // Isi field required dengan data valid kecuali nomor telepon
            $namaCabang = 'Cabang Test ' . time();
            $alamat = 'Alamat Test ' . time();

            $browser->type('input[name="nama"]', $namaCabang)
                ->type('textarea[name="alamat"]', $alamat)
                ->type('input[id="branch_nomor_telepon_display"]', '12345') // Nomor telepon tidak valid
                ->pause(500);

            // Setup jam operasional (required)
            $browser->script('
                let button = document.querySelector("button[onclick=\"bukaSemuaHari()\"]");
                if (!button) {
                    const buttons = document.querySelectorAll("button");
    ...
    /**
     * Negatif: Form cabang harus menolak submit jika email tidak valid
     */
    public function testTambahCabangGagalKarenaEmailTidakValid(): void
    {
        $this->browse(function (Browser $browser) {
            $this->loginAsAdmin($browser);
            ...
            $browser->type('input[name="nama"]', $namaCabang)
                ->type('textarea[name="alamat"]', $alamat)
                ->type('input[id="branch_nomor_telepon_display"]', $nomorTelepon)
                ->type('input[name="email"]', 'invalid-email-format') // Email tidak valid
                ->pause(500);
